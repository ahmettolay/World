  ...
  return (
    <div  className='app'>
            ...
            <Routes>
              ...
              {blogs.map(blog=>{
return ( <Route key={blog.id}> 
  <Route key={blog.id} path={`/components/Blogs/BlogEdit:${blog.id}`} element={<BlogEdit blog={blog}/>}/>
        <Route key={blog.id} path={`/components/Blogs/BlogRead:${blog.id}`} element={<BlogRead blog={blog}/>}/>
        </Route>
)
})}   
              </Route>
